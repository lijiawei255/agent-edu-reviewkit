...
1. 优先扫描 `课程资料/` 目录：使用 `Glob("课程资料/*/")` 发现子文件夹，选择第一个包含课件文件（PDF/PPTX/DOCX）的子文件夹作为课件目录
2. 如果 `课程资料/` 为空或不存在，回退到扫描当前目录下的课件文件
3. 使用 `Glob` 扫描课件目录中所有 PDF/PPTX/DOCX 文件
4. 从文件名中提取章节编号和主题（如 `chapter1_L1_introduction.pdf` → 第1章 绪论）
5. 假设所有找到的文件都在考试范围内
6. 输出路径推断为 `复习文档输出/<课程文件夹名>/`
7. 在生成的 HTML 封面信息中，所有推断值标注 `[自动推断]`
8. 在课件目录中自动生成 `exam-scope-template.json`（含推断的默认值），供用户下次填写

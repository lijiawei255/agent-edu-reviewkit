...
HTML 文件本身包含的 CSS 和 SVG 占用空间有限。课件图片通过相对路径引用，不嵌入 HTML（除非是内联 SVG），因此文件大小可控。如需分享，将 HTML 文件与 `extracted_images/` 文件夹一起打包即可。

### Q: 如何打印复习文档？

HTML 文件内置了 `@media print` 打印样式。在浏览器中打开 HTML 文件，按 `Ctrl+P`（或 `Cmd+P`）即可打印。打印样式会自动优化字体大小、隐藏不必要的界面元素。
